...
local top_accents = {
    ...
    "20DC", "20E1", "20E9", "20F0", "23DC", "23DE", "23E0", "00A8", "00AA",
    "00AF", "00B0", "00B4",
}
...
for _,g in next, chars do
    for c in utfcharacters(g) do
        for _,a in next, top_accents do
            context.mathematics(format([[\Umathaccent fixed "0"0"%s {\Umathchar "0"0 `%s}]], a, c))
            context(" ")
        end
        context.par()
        for _,a in next, bot_accents do
            context.mathematics(format([[\Umathaccent bottom fixed "0"0"%s {\Umathchar "0"0 `%s}]], a, c))
            context(" ")
        end
        context.par()
    end
